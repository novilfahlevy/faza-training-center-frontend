...
export default function PesertaPelatihanPage({ params }) {
  ...
  const fetchPesertaStats = async () => {
    try {
      const res = await fetchPelatihanParticipants(id, { page: 1, size: 999999 });
  ...
  const handleStatusChange = async (pesertaId, newStatus) => {
    ...
    try {
      ...
      // Refresh data peserta dan statistik
      fetchPeserta(activePage, limit, search);
      fetchPesertaStats();
    } catch (error) {
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      const res = await downloadAllCertificates(id);
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", `Sertifikat_${pelatihan?.nama || "Pelatihan"}.zip`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
  ...
  const handleMarkAllAttended = async () => {
    ...
    try {
      const res = await markAllAttended(id);
      toast.dismiss();
            ...
            <div className="block md:hidden divide-y divide-blue-gray-50">
              {pesertaList.length === 0 ? (
                ...
                pesertaList.map((peserta, index) => {
                  ...
                  const isImage = bukti && (bukti.endsWith(".jpg") || bukti.endsWith(".jpeg") || bukti.endsWith(".png"));
                  ...
                  return (
                    <div key={peserta.id} className="p-4 space-y-3 bg-white">
                      ...
                      {bukti && (
                        <div>
                          <Typography variant="small" className="text-gray-500 text-xs mb-1">Bukti Pembayaran:</Typography>
                          {isImage ? (
                            <a href={bukti} target="_blank" rel="noopener noreferrer" className="block w-16 h-16 rounded overflow-hidden border border-blue-gray-100 shadow-sm">
                              <img src={bukti} alt="Bukti" className="w-full h-full object-cover" />
                            </a>
                          ) : isPdf ? (
                            <a href={bukti} target="_blank" rel="noopener noreferrer" className="text-blue-500 text-xs hover:underline">Lihat PDF</a>
                          ) : null}
                        </div>
                      )}
                      <div className="relative">
                        ...
                        {authUser?.role === "admin" ? (
                          ...
                          <Typography className="capitalize">{peserta.status || "-"}</Typography>
                        )}
                      </div>
                    </div>
                  );
