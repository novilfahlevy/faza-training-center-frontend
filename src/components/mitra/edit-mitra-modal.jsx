...
export default function EditMitraModal({ open, onClose, mitra }) {
  ...
  useEffect(() => {
    if (mitra) setForm(mitra);
  }, [mitra]);

  const handleChange = (e) => setForm({ ...form, [e.target.name]: e.target.value });

  const handleSubmit = () => {
    console.log("Update mitra:", form);
    onClose();
  };
  ...
  return (
    <Dialog open={open} handler={onClose} size="md">
      ...
      <DialogFooter>
        ...
        <Button color="blue" onClick={handleSubmit}>Simpan Perubahan</Button>
      </DialogFooter>
    </Dialog>
  );
}
